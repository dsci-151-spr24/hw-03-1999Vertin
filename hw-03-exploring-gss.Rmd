...
```{r}
gss16 %>%
  filter(harass5 == "Yes", na.rm = TRUE) %>%
  nrow()
```
```{r}
gss16 %>%
  filter(harass5 == "No", na.rm = TRUE) %>%
  nrow()
```
```{r}
gss16 %>%
  filter(harass5 != "Yes" ,harass5 != "No", na.rm = TRUE) %>%
  nrow()
```
...
gss16 %>%
  select(harass5) %>%
  filter(harass5 != "Does not apply (i do not have a job/superior/co-worker)", na.rm = TRUE ) %>%
  mutates = mutate(harass5 / sum())
```
...
```{r}
ggplot(data = gss16, aes(x = email, y = )) +
  geom_bar()
